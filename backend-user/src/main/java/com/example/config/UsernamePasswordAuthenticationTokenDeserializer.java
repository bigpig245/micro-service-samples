...
/**
 * This is a fork of the class with the same name in Spring Security. The only difference is the way the principal is
 * deserialized: Spring Security forces a {@link org.springframework.security.core.userdetails.User}, when deserializing
 * as an Object is actually enough since it includes its type.
 */
class UsernamePasswordAuthenticationTokenDeserializer extends JsonDeserializer<UsernamePasswordAuthenticationToken> {
     ...
     */
    @Override
    public UsernamePasswordAuthenticationToken deserialize(JsonParser jp, DeserializationContext ctxt) throws
        ...
        if (principalNode.isObject()) {
            principal = mapper.readValue(principalNode.traverse(), Object.class);
        } else {
            principal = principalNode.asText();
        }
        Object credentials = readJsonNode(jsonNode, "credentials").asText();
        List<GrantedAuthority> authorities = mapper.readValue(
                readJsonNode(jsonNode, "authorities").traverse(), new TypeReference<List<GrantedAuthority>>() {
                });
        if (authenticated) {
